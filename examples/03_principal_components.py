...
found = frame.select(pf.pca(columns).alias("pca")).unnest("pca")
...
standardised = frame.select(pf.pca(columns, scale=True, n_components=3).alias("pca")).unnest("pca")
...
scores = frame.faer.pca_transform(columns, n_components=3)  # type: ignore[attr-defined]
